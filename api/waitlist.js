...
export default async function handler(req, res) {
  ...
  if (!phone || !business) {
    return res.status(400).json({ error: 'Phone and business type are required' });
  }
  ...
  try {
    ...
    const existing = await sql`SELECT id FROM waitlist WHERE phone = ${phone}`;
    if (existing.length > 0) {
      return res.status(400).json({ error: 'already_registered', message: 'This phone is already on the waitlist' });
    }

    // Insert into database
    await sql`
      INSERT INTO waitlist (phone, business)
      VALUES (${phone}, ${business})
    `;
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      await resend.emails.send({
        ...
        to: process.env.NOTIFICATION_EMAIL || 'malleshlingachar1@gmail.com',
